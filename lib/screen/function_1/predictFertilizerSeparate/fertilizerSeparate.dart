...
class FertilizerSeparate extends StatefulWidget {
  const FertilizerSeparate({super.key});

  @override
  _FertilizerSeparateState createState() => _FertilizerSeparateState();
}

// The state class for Leaf, managing the leaf disease analysis process.
class _FertilizerSeparateState extends State<FertilizerSeparate> {
...
// Processes the result from the image analysis and updates the UI.
  void _processResult(Map<String, dynamic> result) async {
    ...
    if (result.containsKey('error')) {
      ...
      recommendationFertilizerList = [];
      recommendationUsageList = [];
    } else {
      ...
      recommendationFertilizerList =
          _getRecommendationsForFertilizer(diseaseClass);
      recommendationUsageList = _getRecommendationsForUsage(diseaseClass);
